...
const TODO_SWR_KEY = "/api/v1/todo"
const fetcher = async () => listTodos()

export function TodoList() {
  const { data, error, isLoading } = useSWR<Todo[]>(TODO_SWR_KEY, fetcher)
  ...
  async function rename(todo: Todo, newTitle: string) {
    ...
    mutate(TODO_SWR_KEY, (curr: any) => (curr || []).map((t: Todo) => (t.id === todo.id ? next : t)), false)
    try {
      await updateTodo(next)
      toast({ title: "Todo updated" })
    } catch {
      mutate(TODO_SWR_KEY) // rollback
  ...
  async function toggle(todo: Todo) {
    ...
    try {
      ...
      mutate(TODO_SWR_KEY) // rollback to revalidate
      toast({ title: "Failed to update", variant: "destructive" as any })
    }
  }
  ...
  async function remove(todo: Todo) {
    ...
    mutate(TODO_SWR_KEY, (curr: any) => (curr || []).filter((t: Todo) => t.id !== todo.id), false)
    const undoId = setTimeout(async () => {
      try {
        await deleteTodo(todo.id)
        toast({ title: "Todo deleted" })
      } catch {
        mutate(TODO_SWR_KEY, prev, false)
...
function TodoSection({
  ...
  items: { id: string | number; title: string; completed: boolean }[]
  onToggle: (t: any) => void
  onDelete: (t: any) => void
